...
Keyword arguments (fall back to custom variables if not provided):
:skeleton   - Skeleton directory (default: `iduh-org-ssg-skeleton-directory').
              Must pass `iduh-org-ssg-verify-skeleton'.
:output     - Output directory for generated HTML (default: `iduh-org-ssg-output-directory').
:posts      - Source directory containing Org files (default: `iduh-org-ssg-posts-directory').
:lang       - HTML language attribute (default: `iduh-org-mode-ssg-default-lang').
:fonts-url  - Google Fonts URL (default: `iduh-org-mode-ssg-google-fonts-url').

This function:
1. Verifies the skeleton directory structure
2. Finds all .org files in POSTS directory
3. Generates HTML for each (with mandatory #+DATE validation)
4. Copies static assets to OUTPUT directory
5. Returns the list of generated files."
  (interactive)
  
  (let* ((skeleton-root (expand-file-name (or skeleton iduh-org-ssg-skeleton-directory)))
         (output-dir (expand-file-name (or output iduh-org-ssg-output-directory)))
         (posts-dir (expand-file-name (or posts iduh-org-ssg-posts-directory)))
