;;; org-ssg.el --- Minimal Static Site Generator for Org-Mode -*- lexical-binding: t; -*-
...
;;
;; Usage:
;;   (org-ssg-generate "input.org" "output.html" "style.css")
;;
;; Or interactively:
;;   M-x org-ssg-generate-interactive
...
(defgroup org-ssg nil
  "Minimal static site generator for Org-Mode."
  :group 'org
  :prefix "org-ssg-")

(defcustom org-ssg-default-lang "en"
  "Default language attribute for generated HTML."
  :type 'string
  :group 'org-ssg)

(defcustom org-ssg-google-fonts-url
...
(define-error 'org-ssg-error "Org-SSG Error")
(define-error 'org-ssg-subheading-error "Sub-heading not allowed" 'org-ssg-error)
(define-error 'org-ssg-image-error "Image syntax error" 'org-ssg-error)
(define-error 'org-ssg-parse-error "Parse error" 'org-ssg-error)

(defun org-ssg--error (type message &rest args)
  "Signal an org-ssg error of TYPE with MESSAGE formatted with ARGS."
...
(defun org-ssg--parse-buffer ()
  ...
  (let ((title nil)
    ...
    (while (not (eobp))
      (let ((line (buffer-substring-no-properties
        ...
        (cond
         ...
         ((string-match "^\\*\\*+[ \t]+" line)
          (org-ssg--error 'org-ssg-subheading-error
         ...
         ((string-match "^[ \t]*\\[\\[\\([^]]+\\)\\]\\][ \t]*$" line)
          (org-ssg--error 'org-ssg-image-error
    ...
    (when in-quote
      (org-ssg--error 'org-ssg-parse-error
...
(defun org-ssg--format-inline (text)
...
(defun org-ssg--escape-html (text)
...
(defun org-ssg--generate-html (doc css-path)
  ...
  (let ((title (or (plist-get doc :title) "Untitled"))
    ...
    (concat
     ...
     "<!DOCTYPE html>\n"
     "<html lang=\"" org-ssg-default-lang "\">\n"
     "<head>\n"
     "  <meta charset=\"UTF-8\">\n"
     "  <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">\n"
     "  <title>" (org-ssg--escape-html title) "</title>\n"
     "  <meta name=\"generator\" content=\"org-ssg\">\n"
     "  <link rel=\"preconnect\" href=\"https://fonts.googleapis.com\">\n"
     "  <link rel=\"preconnect\" href=\"https://fonts.gstatic.com\" crossorigin>\n"
     "  <link href=\"" org-ssg-google-fonts-url "\" rel=\"stylesheet\">\n"
     ...
     "    <header>\n"
     "      <h1 class=\"title\">" (org-ssg--format-inline (org-ssg--escape-html title)) "</h1>\n"
     (if subtitle
         (concat "      <p class=\"subtitle\">"
                 (org-ssg--format-inline (org-ssg--escape-html subtitle))
                 "</p>\n")
       "")
     "    </header>\n"
     ;; Sections
     (mapconcat #'org-ssg--generate-section sections "\n")
     "\n  </article>\n"
     "</body>\n"
     "</html>\n")))

(defun org-ssg--generate-section (section)
  "Generate HTML for a SECTION."
  (let ((heading (plist-get section :heading))
        (content (plist-get section :content)))
    (concat
     "    <section>\n"
     "      <h2>" (org-ssg--format-inline (org-ssg--escape-html heading)) "</h2>\n"
     (mapconcat #'org-ssg--generate-content content "")
     "    </section>")))

(defun org-ssg--generate-content (item)
  "Generate HTML for a content ITEM (paragraph, image, or quote)."
  (let ((type (plist-get item :type)))
    (pcase type
      ('paragraph
       (concat "      <p>"
               (org-ssg--format-inline (org-ssg--escape-html (plist-get item :text)))
               "</p>\n"))
      ('image
       (let ((src (plist-get item :src))
             (alt (plist-get item :alt)))
         (concat "      <figure>\n"
                 "        <img src=\"" (org-ssg--escape-html src) "\" "
                 "alt=\"" (org-ssg--escape-html alt) "\">\n"
                 "        <figcaption>" (org-ssg--escape-html alt) "</figcaption>\n"
                 "      </figure>\n")))
      ('quote
                 ...
                 (mapconcat
                  (lambda (line)
                    (let ((trimmed (string-trim line)))
                      (if (> (length trimmed) 0)
                          (concat "        <p>"
                                  (org-ssg--format-inline (org-ssg--escape-html trimmed))
                                  "</p>\n")
                        "")))
                  lines "")
                 "      </blockquote>\n")))
...
;;;###autoload
(defun org-ssg-generate (input-file output-file css-path)
...
CSS-PATH is the path to the stylesheet (relative to the output HTML)."
  ...
  (unless (file-exists-p input-file)
    (org-ssg--error 'org-ssg-error "Input file does not exist: %s" input-file))
  
  (let* ((doc (org-ssg--parse-file input-file))
         (html (org-ssg--generate-html doc css-path)))
...
;;;###autoload
(defun org-ssg-generate-interactive ()
  "Interactively generate HTML from an Org file."
  (interactive)
  (call-interactively #'org-ssg-generate))

;;;###autoload
(defun org-ssg-preview (input-file)
  "Generate and preview HTML from INPUT-FILE in browser."
  (interactive
   (list (read-file-name "Input Org file: " nil nil t nil
                         (lambda (f) (or (file-directory-p f)
                                         (string-suffix-p ".org" f))))))
  (let* ((temp-html (make-temp-file "org-ssg-preview-" nil ".html"))
    ...
    ;; Generate HTML
    (org-ssg-generate input-file temp-html "style.css")
    ;; Open in browser
